...
museBroadcast.send = function museBroadcast$send(tx, privKeys, callback) {
  var resultP = museBroadcast._prepareTransaction(tx).then(function (transaction) {
    console.log('Signing transaction (transaction, transaction.operations)', transaction, transaction.operations);
    ...
    console.log('Broadcasting transaction (transaction, transaction.operations)', transaction, transaction.operations);
...
_operations2.default.forEach(function (operation) {
  ...
  museBroadcast[operationName + 'With'] = function museBroadcast$specializedSendWith(wif, options, callback) {
    console.log('Sending operation "' + operationName + '" with', { options: options, callback: callback });
    var keys = {};
  ...
  museBroadcast[operationName] = function museBroadcast$specializedSend(wif) {
    ...
    console.log('Parsing operation "' + operationName + '" with', { args: args });
